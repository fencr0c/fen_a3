...

{
    ...
    private _skillAimAccuracy=missionNamespace getVariable["fen_ais_skillAimAccuracy",0];
    if (_skillAimAccuracy>0) then {
        _spnUnt setSkill ["aimingAccuracy",_skillAimAccuracy];
    };
    if ("stop:" in _grpOpt) then {
		...
		private _removeMagazines=missionNamespace getVariable["fen_ais_removeMagazines",[]];
		{
			if (_x in _removeMagazines) then {
